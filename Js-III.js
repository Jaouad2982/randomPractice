...
function sumPetYears(arr, kind, multiply) {
    let petsArray = arr.filter((element)=>{return element.species === kind});
    let map = petsArray.map((element)=>{return element.age * multiply});
    let reduce = map.reduce((total,element)=>{return total+=element});
    return `The combined ${kind}s' ages: ${reduce}`;
} 
